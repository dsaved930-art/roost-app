...
async function renderAuthGate(mode) {
  const c = document.getElementById('auth-gate-content');
  let googleEnabled = false;
  try { const s = await api('/auth/google-status'); googleEnabled = s.enabled; } catch (e) { googleEnabled = false; }

  const googleBtnHtml = googleEnabled
    ? `<button type="button" class="google-btn" id="google-btn">${GOOGLE_ICON_SVG}<span>Continue with Google</span></button><div class="auth-divider"><span>or</span></div>`
    : `<button type="button" class="google-btn" id="google-btn">${GOOGLE_ICON_SVG}<span>Continue with Google</span></button><div class="auth-divider"><span>or</span></div>`;
  ...
  document.getElementById('google-btn').addEventListener('click', () => {
    if (googleEnabled) { window.location.href = '/api/auth/google'; return; }
    const err = document.getElementById('auth-error');
    err.style.color = 'var(--muted)';
    err.textContent = "Google sign-in isn't configured on this deployment yet — see the backend README for setup steps.";
  });
}
